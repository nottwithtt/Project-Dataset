...
const connection = mysql.createConnection(DATABASE_URL='mysql://95egejlogqswtzpwvkaz:pscale_pw_qPtgcawlNqCYxrG5YellmUjYf94UgHknYZOinjn5pkW@aws.connect.psdb.cloud/db2-project1?ssl={"rejectUnauthorized":true}');
...
app.post("/createComment",bodyParser.json(), async (req,res)=>{
    ...
    let p_idComment = req.body.idComment;
    ...
    let queryCreate = `INSERT INTO comment (idUser, idCommentResponse, idDataset, creationDate, content, file) VALUES("${p_idUser}", ${p_idCommentResponse}, "${p_idDataset}", "${p_creationDate}", "${p_content}", ${p_file})`;
